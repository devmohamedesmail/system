...
export default function Checks() {
  ...
  const columns = [
    ...
    },
    {
        ...
        cell:(row)=>(
            <div>
                <CustomActionButton onpress={()=>deletecheck(row)} icon={<MdDelete color="red" size={20} />} />
            </div>
        )
    },
  ]
  ...
  return (
    <div className="p-2">
      ...
      <div className="bg-white my-3 p-2">
        ...
        <div>
          <CustomButton title={t("add")} onpress={() => handleAddCheck()} />
        </div>
      
      </div>
      <div className="my-3 bg-white p-2">
        <div className="flex items-center">
          <CustomTableButton title={t('download')} onclick={()=>handleExportChecks()} />
        ...
        <DataTable columns={columns} data={checks} pagination  selectableRows fixedHeader />
      </div>
    </div>
  );
}
